set(LIBZ_SRC_DIR "${PROJECT_SOURCE_DIR}/contrib/libz")
set(LIBZ_BUILD_DIR "${PROJECT_BINARY_DIR}/contrib/libz")
set(LIBZ_LIB "${LIBZ_BUILD_DIR}/libz.a")

ExternalProject_Add(
    libz-external
    BINARY_DIR ${LIBZ_BUILD_DIR}
    DOWNLOAD_COMMAND ""
    CONFIGURE_COMMAND /bin/sh -c "${EBPF_EXT_ENV_FLAGS} CFLAGS=-fPIE ${LIBZ_SRC_DIR}/configure --static"
    BUILD_COMMAND /bin/sh -c "${EBPF_EXT_ENV_FLAGS} make"
    INSTALL_COMMAND ""
    BUILD_IN_SOURCE 0
    BUILD_BYPRODUCTS ${LIBZ_LIB}
)
add_library(libz STATIC IMPORTED GLOBAL)
set_property(TARGET libz PROPERTY IMPORTED_LOCATION "${LIBZ_LIB}")
set_property(TARGET libz PROPERTY INTERFACE_INCLUDE_DIRECTORIES "${LIBZ_BUILD_DIR}")
add_dependencies(libz libz-external)
